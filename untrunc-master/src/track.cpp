...
int64_t Track::getNumSamples() const {
	if (sizes_.size()) assertt(num_samples_ == sizes_.size());
	return num_samples_;
}
...
void Track::saveChunkOffsets() {
	...
	assertt(chunks_[0].off_ >= 0, codec_.name_, chunks_[0].off_);
...
void Track::genLikely() {
	...
	assertt(sizes_.size() > 0 || constant_size_);
...
void Track::genChunkSizes() {
	...
	assertt(chunks_[0].n_samples_ >= 1);
