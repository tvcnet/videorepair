...
extern uint g_max_partsize, // max theoretical part size
...
extern bool g_interactive, g_muted, g_ignore_unknown, g_stretch_video,
    ...
    g_rsv_ben_mode, g_dump_repaired, g_search_mdat, g_strict_nal_frame_check,
    g_allow_large_sample, g_ignore_forbidden_nal_bit, g_noise_buffer_active,
    g_dont_omit, g_ignore_keyframe_mismatch, g_skip_nal_filler_data,
    g_off_as_hex, g_fast_assert, g_ignore_out_of_bound_chunks, g_skip_existing,
    g_no_ctts, g_is_gui;
...
#define logg(lvl, ...)                                                         \
  do {                                                                         \
    if (g_log_mode >= lvl) {                                                   \
      _logg(lvl, __VA_ARGS__);                                                 \
    } else if (lvl == W2) {                                                    \
      g_num_w2++;                                                              \
    }                                                                          \
  } while (0)

template <class... Args> void _logg(Args &&...args) {
...
template <class... Args> void _logg(LogMode m, Args &&...x) {
  if (m == I)
    std::cout << "Info: ";
  else if (m == W || m == W2)
    std::cout << "Warning: ";
...
#define loggF(lvl, ...)                                                        \
  do {                                                                         \
    if (g_log_mode >= lvl) {                                                   \
      _loggF(lvl, __VA_ARGS__);                                                \
    } else if (lvl == W2) {                                                    \
      g_num_w2++;                                                              \
    }                                                                          \
  } while (0)

template <class... Args> void _loggF(Args &&...x) {
  if (g_noise_buffer_active) {
    disableNoiseBuffer();
    _logg(std::forward<Args>(x)...);
    enableNoiseBuffer();
...
bool contains(const std::initializer_list<std::string> &c,
              const std::string &v);
...
C<T> choose100(const C<T> &in) {
  ...
  std::uniform_int_distribution<size_t> dis(
      0, std::distance(in.begin(), in.end()) - 1 - k);
  C<T> out;
...
template <class Numeric> std::string hexIf(Numeric x) {
  if (g_off_as_hex)
    return ss(std::hex, "0x", x, std::dec);
  return ss(x);
}
...
bool findOrder(std::vector<std::pair<int, int>> &data,
               bool ignore_first_failed = false);
...
public:
  ...
  static void editHeaderAtom(Atom *header_atom, int64_t duration,
                             bool is_tkhd = false);
  void editHeaderAtom();
  void readHeaderAtom();
  int getDurationInMs();
...
void printArgs(std::ostream &os, const std::string &sep,
               const std::string &argNames, Args... args) {
...
#define dbgg(msg, ...)                                                         \
  if (g_log_mode >= V)                                                         \
  __dbgg(msg, #__VA_ARGS__, ##__VA_ARGS__)

template <typename... Args>
void __dbgg(const std::string &message, const std::string &argNames,
            Args... args) {
  std::cout << message;
...
#define assertt(Expr, ...)                                                     \
  if (!(Expr))                                                                 \
  __assertt(#Expr, __PRETTY_FUNCTION__, __FILE__, __LINE__, #__VA_ARGS__,      \
            ##__VA_ARGS__)
#else
#define assertt(...) ;
#endif
...
template <typename... Args>
void __assertt(const char *expr_str, const char *fn, const char *file, int line,
               const std::string &argNames, Args... args) {
  disableNoiseBuffer();

  std::cerr << file << ":" << line << ": " << fn << ": Assertion `" << expr_str
            << "' failed.";
