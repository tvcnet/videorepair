...
private:
	...
	bool currentChunkFinished(int add_extra=0) {
		...
		assertt(cur_track_idx == last_track_idx_, cur_track_idx, getCodecName(cur_track_idx), last_track_idx_, getCodecName(last_track_idx_));
...
public:
	...
	void operator++() {
		...
		if (off >= mdat_end_) {
			assertt(g_ignore_out_of_bound_chunks);
...
private:
	...
	ChunkIt(bool is_end_it_) { assertt(is_end_it_); becomeEndIt(); }
